...
def forgeDetection():
    ...
    while True:
        ...
        if ret:
            ...
            # Assume first class (index 0) is "Forgery" and others are "Real"
            actual_labels.append(0 if "forge" in filename.lower() else 1)
    ...
    # Calculate metrics for the selected video
    if len(actual_labels) > 0:
        precision = precision_score(actual_labels, predicted_labels, average='macro') * 100
        recall = recall_score(actual_labels, predicted_labels, average='macro') * 100
        f1 = f1_score(actual_labels, predicted_labels, average='macro') * 100
        accuracy = accuracy_score(actual_labels, predicted_labels) * 100
        
        text.insert(END, f"Metrics for {filename}:\n")
        text.insert(END, f"Accuracy  : {accuracy:.2f}%\n")
        text.insert(END, f"Precision : {precision:.2f}%\n")
        text.insert(END, f"Recall    : {recall:.2f}%\n")
        text.insert(END, f"F1 Score  : {f1:.2f}%\n\n")

    # Save the real frames video
